...
const int button_start_1_pin = 36; //Пин за бутон - програма 1 минута.
const int button_start_4_pin = 24; //Пин за бутон - програма 4 минута.
const int button_start_6_pin = 28; //Пин за бутон - програма 6 минута.
const int button_start_8_pin = 32; //Пин за бутон - програма 8 минута.
const int button_stop_pin = 40;    //Пин за бутон - стоп програма.

const int r_sensor_pin = 31;      //Пин за сензор запълнена ролка на десен мотор. 
const int l_sensor_pin = 37;      //Пин за сензор запълнена ролка на ляв мотор.
const int rele_air_pin = 52;      //Пин за реле въздух.
const int rele_motor_power = 44;//Пин реле мотори за шкурка.
...
const int ratio = 27;            // с намаляне на стойноста забързваме (за скоростта на ляв и десен мотор )  
...
pinMode(rele_motor_power, OUTPUT);
      ...
      if (mode == 1){ //reaching needed rpm        
         ...
         if (millis() < start_time_ms + term_ms){
             ...
             if( /*int(millis())*/i % ratio == 0 ){
                 ...
                 if(s_direction == 0){
                     digitalWrite(rele_motor_power, HIGH);
                     make_step(r_pul_pin, 0);
                     make_step(l_pul_pin, 0);
                 }else{
                     digitalWrite(rele_motor_power, LOW);
                     make_step(r_pul_pin, 0);
                     make_step(l_pul_pin, 0);
                 }
              }
